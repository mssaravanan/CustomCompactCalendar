...
public class CompactCalendarView extends View {
    ...
    public void setSelectorDayCurrentMonth(boolean isCurrentMonthDaySelector){
        compactCalendarController.setSelectorDayCurrentMonth(isCurrentMonthDaySelector);
    }

    public void setSelectedDate(int dayofMonth){
        compactCalendarController.setSelectedDate(dayofMonth);
        invalidate();
    }

    public final GestureDetector.SimpleOnGestureListener gestureListener = new GestureDetector.SimpleOnGestureListener() {
        @Override
    ...
    public void setServerDate(Date dateTimeMonth){
        compactCalendarController.setTodaySeverDate(dateTimeMonth);
        invalidate();
    }
    ...
    public void setCurrentSelectedDayIndicatorStyle(final int currentSelectedDayIndicatorStyle){
        compactCalendarController.setSelectedDayIndicatorStyle(currentSelectedDayIndicatorStyle);
        invalidate();
    }
        ...
        try {
           ...
           switch (event.getAction()) {
            ...
            if (horizontalScrollEnabled) {
                ...
                invalidate();
               /* if (deltaX > 0&&distance) {
                    if (compactCalendarController.getCalendarOfCurrentMonth().get(Calendar.YEAR) != compactCalendarController.maxYear && compactCalendarController.getCalendarOfCurrentMonth().get(Calendar.MONTH) != Calendar.DECEMBER) {

                    }
                }
                if (deltaX < 0&&distance) {
                    if (compactCalendarController.getCalendarOfCurrentMonth().get(Calendar.YEAR) != compactCalendarController.minYear && compactCalendarController.getCalendarOfCurrentMonth().get(Calendar.MONTH) != Calendar.JANUARY) {
                        compactCalendarController.onTouch(event);
                        invalidate();
                    }
                }*/
